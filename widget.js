window.addEventListener("onWidgetLoad", function (obj) {
  ...
  timerElement.style.textShadow = fieldData.textShadow
    ? "0 0 6px black"
    : "none";
  ...
  let countdownInterval = setInterval(function () {
    if (countdownTime >= 0) {
      timerElement.textContent =
        initialMessage + " " + formatTime(countdownTime);
      countdownTime--;
    } else {
  ...
  window.addEventListener("onEventReceived", function (obj) {
    ...
    const isBroadcaster =
      obj.detail.event.data.tags.badges &&
      obj.detail.event.data.tags.badges.includes("broadcaster/1");
